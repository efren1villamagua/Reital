...
import efren.util.entidades.SecClase;
import efren.util.gui.table.DataTableColumn;

public class ClaseABMView extends javax.swing.JInternalFrame implements efren.util.gui.table.DataTablePanelListener, efren.util.gui.text.TextFieldExtListener, java.beans.PropertyChangeListener, javax.swing.event.InternalFrameListener {
	/**
	 *
	 */
 ...
 */
public ClaseABMView(String title) {
	super(title);
}
public void _cerrar() {
...
/**
 * connEtoC5:  (DataTablePanel.selectedObjectFromDoubleClick --> ClaseABMView.modificar()V)
...
public void dataFetch() {
	...
	try {
		...
		String s = " SELECT OID, NOMBRE AS NOMBRE, PAQUETE AS PACKAGE, TIMESTAMP "
					+ " FROM " + SystemProperties.SCHEMA_SEGURIDADES+"." + "CLASE ";
		if (porNombre)
			s = s + " WHERE RTRIM(LTRIM(UCASE(NOMBRE))) LIKE "+getTextFieldExtNombre().SQLFullText()+" ";
		...
		efren.util.entidades.SecClase mb;
		while (rs.next()) {

			mb = new efren.util.entidades.SecClase();

			mb.setOid(rs.getLong(1));
			mb.setNombre(rs.getString(2).trim());
			mb.setPaquete(rs.getString(3).trim());
			mb.setTimestamp(rs.getTimestamp(4));
...
private void eliminar() throws Exception {
    ...
    ClaseABMDetailsView view = new ClaseABMDetailsView();

    view.setObserverThis(getobserver());
    view.mainView = this;

    efren.util.abm.estados.ABMEstado estado = new efren.util.abm.estados.ABMEstadoEliminado("ELIMINAR clase en SEGURIDADES");
    view.setAbmEstado(estado);

    efren.util.entidades.SecClase bo = (efren.util.entidades.SecClase) getDataTablePanel().getSelectedObject();
...
private efren.util.gui.table.DataTablePanel getDataTablePanel() {
	if (ivjDataTablePanel == null) {
			...
			columnsDefinition.add(new DataTableColumn("Package", 270, "Paquete", false, "PACKAGE"));
			columnsDefinition.add(new DataTableColumn("Nombre de clase", 200, "Nombre", false, "NOMBRE"));
			ivjDataTablePanel.setColumnsDefinition(SecClase.class, columnsDefinition);
	}
...
private void initialize() {
	try {
		// user code begin {1}
		// user code end
		setName("ClaseABMView");
		setDefaultCloseOperation(javax.swing.WindowConstants.DO_NOTHING_ON_CLOSE);
		setSize(558, 410);
		setTitle("Clases en SEGURIDADES");
...
private void modificar() throws Exception {
    ...
    efren.util.abm.estados.ABMEstado estado = new efren.util.abm.estados.ABMEstadoModificado("MODIFICAR clase en SEGURIDADES");
    view.setAbmEstado(estado);
...
private void nuevo() throws Throwable {
    ...
    efren.util.abm.estados.ABMEstado estado = new efren.util.abm.estados.ABMEstadoNuevo("NUEVA clase en SEGURIDADES");
    view.setAbmEstado(estado);

    efren.util.entidades.SecClase bo = new efren.util.entidades.SecClase();
