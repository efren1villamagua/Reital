...
public class Conn {
	...
	public static java.sql.Connection conectar() {
		try {
			...
			/*
			 * Thread aThread = new Thread(new Runnable() { public void run() {
			 * try {
			 * 
			 * Font fontOriginal = new java.awt.Font("Arial", Font.BOLD, 10);
			 * Color foregroundOriginal = new java.awt.Color(72,61,139);
			 * 
			 * String mensajeNuevo = "ERROR EN LA CONEXION: "
			 * +Constantes.BARRA_STATUS_MENSAJE; Font fontNuevo = new
			 * Font("Arial", Font.BOLD, 12); Color foregroundNuevo = Color.red;
			 ...
			 * 
			 * String mensajeAbajo =
			 * Constantes.DATA_STORE_NAME+"»"+Constantes.SERVER_NAME+"»"+
			 * SystemProperties.RUNTIME_USER_NAME+"»"+SystemProperties.
			 * RUNTIME_NOMBRE_USUARIO
			 * +"»(JDK"+System.getProperty("java.version")+"»"+System.
			 * getProperty("os.name")
			 * +"»"+System.getProperty("os.arch")+"»"+System.getProperty(
			 * "os.version") +"»"+java.net.InetAddress.getLocalHost()+")»" +
			 * "Conectado desde: "
			 * +efren.util.Constantes.INIT_CONECTION_CALENDAR_MANAGER.
			 * getAbsoluteRegionalDateExpression();
			 * 
			 ...
			 * catch (Exception e) { e.getMessage(); } } }); aThread.start();
			 */
			...
			try {
				...
				if (SystemView.singleton() != null)
					opcionEscogida = InfoView.showConfirmDialog(SystemView.singleton(),
							"La conexión al Servidor no es válida. Desea seguir intentando re-conectarse?");
				else {
					if (SystemAdminView.singleton() != null)
						opcionEscogida = InfoView.showConfirmDialog(SystemAdminView.singleton(),
								"La conexión al Servidor no es válida. Desea seguir intentando re-conectarse?");
					else
						opcionEscogida = InfoView.showConfirmDialog(new JFrame(),
								"La conexión al Servidor no es válida. Desea seguir intentando re-conectarse?");
				}
				// if (reconectarseEnCasoError && opcionEscogida ==
				// InfoView.YES_OPTION) {
				if (opcionEscogida == InfoView.YES_OPTION) {
					// if (sqle.getMessage() !=null &&
					// sqle.getMessage().indexOf("CLI06") > 0) {
					try {
						con.close();
					} catch (Throwable t) {
						t.getMessage();
					...
					if (SystemProperties.SERVER_TYPE == Constantes.SERVER_TYPE_1_OS400) {// as400
						...
						url = "jdbc:as400://" + SystemProperties.DATA_STORE_IP + ";prompt=false;transaction isolation=read uncommitted;libraries="
